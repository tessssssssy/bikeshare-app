...
class BookingsController < ApplicationController
  ...
  # create a booking
  def create    
    @booking = Booking.new do |booking|
    ...
    # for some reason it would not let me create a booking using booking_params and also add the listing_id, so I had to manually add each attribute
    ...
    if @booking.start_date.nil? || @booking.end_date.nil?
      ...
      redirect_to new_listing_booking_path
      ...
      if @booking.save
        if current_user.id == @listing.user_id
          ...
          redirect_to "/listings/#{@listing.id}/bookings/#{@booking.id}"
          end
      else
        render :new
      end
